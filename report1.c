...
int main(void){
    struct report1 name = {"김형원", 20160195};
...
// 구조체 별칭을 사용하여 포인터 선언, book *p1과 같이 구조체 별칭으로 포인터를 선언 한 뒤 malloc 함수로 메모리를 할당한다. 이때 할당할 메모리 크기도 sizeof(book)처럼 구조체 별칭으로 구할 수 있다.
// 구조체 포인터에서 멤버에 접근하려면 p1->name과 같이 화살표 연산자를 사용한다. 괄호와 역참조를 사용하면 .(점)으로 멤버에 접근할 수 있다.
...
// int main(){
//     Student wizard[3] = {{.name = "해리 포터"}, {.name = "헤르미온느"}, {.name = "론 위즐리"}};

//     wizard[0].age = 17;
//     wizard[1].age = 16;
//     wizard[2].age = 17;

//     printf("이름 : %s / 나이 : %d\n", wizard[0].name, wizard[0].age);
//     printf("이름 : %s / 나이 : %d\n", wizard[1].name, wizard[1].age);
//     printf("이름 : %s / 나이 : %d\n", wizard[2].name, wizard[2].age);
